...
while True:
    ...
    if verify == 1:
        ...
        #conversor númerico para outras moedas
        moeda = int(input("Digite qual moeda você deseja cadastrar: \n1-Real\n2-Dólar\n3-Euro\n"))
        while True:
          if moeda == 1:
            produto.append("R$")
            break
        ...
        for i, produto in enumerate(matriz):
            if produto[2] < 5:
                print(f"Produto {i+1} Com Estoque Baixo - Nome: {produto[0]}, Categoria: {produto[1]}, Quantidade: {produto[2]}, Preço: {produto[3]}{produto[4]}, Valor Total: {produto[5]}")
            else:
                print(f"Produto {i+1} - Nome: {produto[0]}, Categoria: {produto[1]}, Quantidade: {produto[2]}, Preço: {produto[3]}{produto[4]}, Valor Total: {produto[5]}")
